...
func snapcommon(t *testing.T, name string, disconnect bool, reliable bool, crash bool) {
	...
	cfg.one(rand.Int(), servers, true)
	leader1 := cfg.checkOneLeader()
	fmt.Printf("[Test] One entry replicated (leader is %v)\n", leader1)
	...
	for i := 0; i < iters; i++ {
		...
		if disconnect {
			fmt.Printf("[%v][Test] Disconnecting victim %v\n", i, victim)
			cfg.disconnect(victim)
			cfg.one(rand.Int(), servers-1, true)
			fmt.Printf("[%v][Test] Entry replicated after disconnecting %v\n", i, victim)
		}
		if crash {
			fmt.Printf("[%v][Test] Crashing victim %v\n", i, victim)
			cfg.crash1(victim)
			cfg.one(rand.Int(), servers-1, true)
			fmt.Printf("[%v][Test] Entry replicated after crash\n", i)
		}
		// send enough to get a snapshot
		for i := 0; i < SnapShotInterval+1; i++ {
			cfg.rafts[sender].Start(rand.Int())
		}
		fmt.Printf("[%v][Test] %v entries sent to leader %v\n", i, SnapShotInterval+1, sender)
		...
		if disconnect {
			...
			fmt.Printf("[%v][Test] Connecting victim %v\n", i, victim)
			cfg.connect(victim)
			cfg.one(rand.Int(), servers, true)
			fmt.Printf("[%v][Test] Entry replicated after connecting %v\n", i, victim)
			leader1 = cfg.checkOneLeader()
		}
		if crash {
			fmt.Printf("[%v][Test] Uncrashing victim %v\n", i, victim)
			cfg.start1(victim, cfg.applierSnap)
			cfg.connect(victim)
			cfg.one(rand.Int(), servers, true)
			fmt.Printf("[%v][Test] Entry replicated after uncrashing %v\n", i, victim)
			leader1 = cfg.checkOneLeader()
		}
	}
	cfg.end()
